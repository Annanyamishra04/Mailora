...
import { validateActionItemsRequest } from "@/lib/validation/ai";

// Gemini calls are capped at 20s in the provider; leave headroom for auth + validation.
export const maxDuration = 30;

/**
 * POST /api/ai/action-items — requires a signed-in user (401 otherwise), is rate
...
export const POST = createAiRoute({
  tag: "api/ai/action-items",
  validate: validateActionItemsRequest,
  run: extractActionItems,
});
